...
import com.epam.controller.BookApi;
...
class TestBookController {
	...
	void testGetBookWithMethodArgumentTypeMismatchException() throws JsonProcessingException, Exception {
		BookDto bookDto = new BookDto(1,"oko","oko","oko",1);
		when(bookService.getBook(1)).thenReturn(bookDto);
...
//		BookDto bookDto=new BookDto();
	...
	void testUpdateQuestion() throws JsonProcessingException, Exception {
		BookDto bookDto = new BookDto(1,"oko","oko","oko",1);
		when(bookService.updateBook(Mockito.any(BookDto.class))).thenReturn(bookDto);
		mockMvc.perform(put("/books").contentType(MediaType.APPLICATION_JSON)
				.content(new ObjectMapper().writeValueAsString(bookDto))).andExpect(status().isAccepted());
	}
}
