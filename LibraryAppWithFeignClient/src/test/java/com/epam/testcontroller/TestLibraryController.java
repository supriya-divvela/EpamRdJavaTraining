...
class TestLibraryController {
	...
	void testGetBook() throws Exception {
		...
		mockMvc.perform(get("/library/books/{bookId}",1)).andExpect(status().isOk()).andReturn();
	...
	void testGetAllUsers() throws Exception {
		...
		when(userFeignClient.getAllUsers()).thenReturn(new ResponseEntity<>(userDtos,HttpStatus.OK));
