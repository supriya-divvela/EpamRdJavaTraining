...
//		QuizDto quizDto = new QuizDto();
		...
		mockMvc.perform(get("/deletequiz").param("quizNo", "2")).andExpect(status().isOk())
				.andExpect(view().name("quizpage")).andExpect(model().attribute("listOfQuizs", quizes))
				.andExpect(model().attribute("deletequizmessage", "Quiz Deleted Succesfully..."));
	}
...
//		QuizDto quizDto=new QuizDto();
		...
		List<QuizDto> quizes = new ArrayList<>();
		when(quizService.displayAvaliableQuizes()).thenReturn(quizes);
		mockMvc.perform(get("/quizpage").param("quizNo", "2")).andExpect(status().isOk())
				.andExpect(view().name("quizpage")).andExpect(model().attribute("listOfQuizs", quizes))
				.andExpect(model().attribute("emptyquizlibrary", "Empty quiz library..."));
