...
class TestQuestionApi {
	...
	@Test
	void testDeleteQuestionWithException() throws JsonProcessingException, Exception {
		doThrow(QuestionException.class).when(questionService).removeQuestion(1);
		mockMvc.perform(delete("/questionspage/questions/{id}", 1).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	}

	@Test
	void testDeleteQuestionWithSQLIntegrityConstraintViolationException() throws JsonProcessingException, Exception {
		doThrow(SQLIntegrityConstraintViolationException.class).when(questionService).removeQuestion(1);
//		when(questionService.removeQuestion(1)).thenThrow(SQLIntegrityConstraintViolationException.class);
		mockMvc.perform(delete("/questionspage/questions/{id}", 1).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isNotFound());
	}
