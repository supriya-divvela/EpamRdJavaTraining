...
public class EmployeeService extends EmployeeData {
	...
	public static void getAverageAgeOfMaleAndFemaleEmployees() {
		System.out.println("Average age of male employees are : "
				+ employeeList.stream().filter(employeeList -> employeeList.getGender().equalsIgnoreCase("male")).mapToInt(Employee::getAge).average().getAsDouble());
//						.collect(Collectors.averagingDouble(Employee::getAge)));
	...
	public static void findMaxSalaryEmployeeDetails() {
		List<Employee> maxSalaryEmployeeDetails = 
//				employeeList.stream().sorted(Collections.reverseOrder(Comparator.comparing(Employee::getSalary))).limit(1);
				employeeList.stream()
		...
		employeeList.stream().sorted(Collections.reverseOrder(Comparator.comparing(Employee::getSalary))).limit(1).forEach(System.out::println);
	}
	...
	public static void getAverageAndTotalSalaryOfOrganisation() {
		double totalSalary = employeeList.stream().mapToDouble(Employee::getSalary).sum();//.collect(Collectors.summingDouble(Employee::getSalary));
	...
	public void getOldestEmployeeDetails() {
		...
		if (oldestEmployeesList.isEmpty()) {
			...
			System.out.println("Oldest employee in the organisation is " + oldestEmployeesList);
