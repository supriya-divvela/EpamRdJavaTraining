...
public class QuizServiceUIImplementation implements QuizServiceUI {
	...
	public Map<Integer, Runnable> getQuizAccess() {
		...
		quizAccess.put(2, () -> {
			...
			try {
				if (quizService.findQuiz(quizNo)) {
					int quizNumber=quizService.removeQuiz(quizNo);
					logger.info("{} Quiz Removed Successfully..",quizNumber);
				} else {
	...
	public Map<Integer, Consumer<Integer>> getUpdateQuiz() {
		...
		updateQuiz.put(2, quizNo -> {
			...
			} catch (QuizNotFoundException | QuestionNotFoundException | DuplicateQuizNumberException e) {
				logger.info(e);
			}
		});
		return updateQuiz;
