...
public class RestExceptionHandler {
	...
	public ExceptionResponse handleMethodArgumentNotValidException(MethodArgumentNotValidException exception,
			WebRequest request) {
		log.error("BookRestExceptionHandler:handleMethodArgumentNotValidException");
	...
	public ExceptionResponse handleQuestionException(BookException exception, WebRequest request) {
	...
	@ResponseStatus(HttpStatus.BAD_REQUEST)
	public ExceptionResponse handleSQLIntegrityConstraintViolationException(SQLIntegrityConstraintViolationException exception,
			WebRequest request) {
		log.info("BookRestExceptionHandler:handleSQLIntegrityConstraintViolationException");
		return new ExceptionResponse(new Date().toString(), HttpStatus.BAD_REQUEST.name(),
				"Cannot add book as same book with that name already exists", request.getDescription(false));
	}
