...
class TestQuizApi {
	...
	@Test
	void testDeleteQuizWithException() throws JsonProcessingException, Exception {
		doThrow(QuizException.class).when(quizService).removeQuiz(1);
//		when(quizService.removeQuiz(1)).thenThrow(QuizException.class);
		mockMvc.perform(delete("/quizspage/quizs/{id}", 1).contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk());
	}
